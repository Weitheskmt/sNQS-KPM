...
cdef cnp.ndarray[cnp.int32_t, ndim=2] __make_basis_idxs_cy(int N):
    cdef:
        Py_ssize_t i, j
        Py_ssize_t num_threads
        cnp.ndarray[cnp.int32_t, ndim=1] d = 1 << np.arange(N, dtype=np.int32)
        cnp.ndarray[cnp.int32_t, ndim=2] out = np.empty((2**N, N), dtype=np.int32)

    num_threads = <Py_ssize_t>(2**N)
...
@cython.boundscheck(False)  # turn off bounds-checking
@cython.wraparound(False)  # turn off negative index wrapping
cdef cnp.ndarray[cnp.int64_t, ndim=2] __make_basis_idxs_int64_cy(int N):
    cdef:
        Py_ssize_t i, j
        Py_ssize_t num_threads
        cnp.ndarray[cnp.int64_t, ndim=1] d = 1 << np.arange(N, dtype=np.int64)
        cnp.ndarray[cnp.int64_t, ndim=2] out = np.empty((2**N, N), dtype=np.int64)
