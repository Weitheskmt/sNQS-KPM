...
def get_Hij_cy(state_i_idx, _unique_XY_sites_idx, _unique2all_XY_sites_idx,
    ...
    state_i_idx = state_i_idx.astype(np.int64)
    ...
    if couplings.dtype == np.float32:
        if state_i_idx.dtype == np.int16:
            H_ij = __inner_int16_float(M, Kxy, K,
                                _unique2all_XY_sites_idx.astype(np.int64),
                                 P_k_by_unique_YZ_sites.astype(np.int64),
                                 _unique2all_YZ_sites_idx.astype(np.int64),
                                 couplings,
                                 H_ij)
